...
function xorBytesWithKey(bytes: number[], keyBytes: number[]): number[] {
...
function base64ToBytes(base64Str: string): number[] {
...
function bytesToString(byteArr: number[]): string {
    const decoder = new TextDecoder();
    return decoder.decode(new Uint8Array(byteArr));
}

function encrypt(strToEncrypt: string, key: string): string {
...
function decrypt(strToDecrypt: string, key: string): number[] {
...
export function encodeSecretString(passcode: string, puzzleData: any): string {
    passcode = passcode.toLowerCase();
    return passcodeHash(passcode) + encrypt(JSON.stringify(puzzleData), passcode);
}

export function decodeSecretString(passcode: string, secretString: string): any {
    passcode = passcode.toLowerCase();
